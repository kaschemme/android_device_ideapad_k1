                              ...
                              int enable)
{
	...
	ALOGD("Setting %s",route[i].ctl_name);
        ctl = mixer_get_ctl_by_name(mixer, route[i].ctl_name);
        if (!ctl)
            return -EINVAL;

        if (route[i].strval) {
		ALOGD("    to %s",route[i].strval);
            if (enable)
                mixer_ctl_set_enum_by_string(ctl, route[i].strval);
            else
                mixer_ctl_set_enum_by_string(ctl, "Off");
        } else {
		ALOGD("    to %d",route[i].intval);
...
static int start_output_stream(struct tf101_stream_out *out)
{
    ...
    ALOGD("start_output_stream: card:%d, port:%d, rate:%d, format:%d",card,port,out->config.rate,out->config.format);
...
{
//ALOGD("Returning out get %d",AUDIO_CHANNEL_OUT_STEREO);
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        if (((adev->devices & AUDIO_DEVICE_OUT_ALL) != val) && (val != 0)) {
            ...
            ALOGD("Headphone out:%c, Speaker out:%c, HDMI out:%c\n",
                         ...
                         size_t bytes)
{
    ...
    /*ALOGD("out_write: writing:%d 0x%02x 0x%02x 0x%02x\n", out_frames * frame_size,
...
#define get_ctl(v,n) do { \
    ...
    if (!adev->mixer_ctls.v) { \
        ALOGE("Unable to find '%s' mixer control", #n); \
        goto error_out; \
    } \
} while (0)
